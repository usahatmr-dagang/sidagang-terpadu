...
export default function App() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser && currentUser.email) {
        try {
          ...
          // ── Muat data SKB & Tarif Khusus dari Firestore (agar tidak hilang saat refresh) ──
          try {
          ...
          try {
            const trRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'tradeRates');
            const trSnap = await getDoc(trRef);
            if (trSnap.exists() && trSnap.data().data) {
              setTradeRates(trSnap.data().data);
            }
          } catch (e) { /* abaikan jika gagal */ }
  ...
  const [newTradeRate, setNewTradeRate] = useState({ jenisUsaha: '', billingDays: 'EVERYDAY', tarifWeekday: 10000, tarifWeekend: 15000 });
  ...
  const saveSpecialDates = async (newDates) => {
    setSpecialDates(newDates);
    try {
      const sdRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'specialDates');
      await setDoc(sdRef, { data: newDates, updatedAt: new Date().toISOString() });
    } catch (e) {
      console.error("Gagal menyimpan kalender ke Cloud:", e);
    }
  };

  const handleSaveTradeRates = async (newRates) => {
    try {
      const trRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'tradeRates');
      await setDoc(trRef, { data: newRates, updatedAt: new Date().toISOString() });
      setTradeRates(newRates);
      showToast("Tarif khusus jenis dagangan berhasil disimpan ke Cloud.", "success");
    } catch (e) {
      showToast("Gagal menyimpan tarif khusus ke Cloud.", "error");
    }
  };

  const saveCalendarDate = async (e) => {
    e.preventDefault();
    ...
    await saveSpecialDates(newDates); 
  ...
  const getKalkulasiBulan = (bulan, tahun) => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      if (statusSpesial === 'LIBUR' || statusSpesial === 'CUTI' || statusSpesial === 'PEAK') hariRamai++; 
  ...
  const calculateMerchantBill = (merchant, dates = specialDates, rules = tradeRates, bulan = calMonth, tahun = calYear) => {
    if (merchant.tipeTarif === 'TETAP') {
      return merchant.tagihanTetapBulanan || 0;
    }

    // Find if there is a matching trade rate rule
    const matchRule = rules.find(rule => 
      merchant.jenisUsaha && 
      rule.jenisUsaha &&
      merchant.jenisUsaha.trim().toUpperCase().includes(rule.jenisUsaha.trim().toUpperCase())
    );
    ...
    if (matchRule) {
      weekdayRate = Number(matchRule.tarifWeekday) ?? 10000;
      weekendRate = Number(matchRule.tarifWeekend) ?? 15000;
      effectiveTipeTarif = matchRule.billingDays; // Override schedule!
    }
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const statusSpesial = dates[dateStr]?.type;

      let isWeekendOrHoliday = false;
      let isTutup = false;

      // Determine day category
      if (statusSpesial === 'LIBUR' || statusSpesial === 'CUTI' || statusSpesial === 'PEAK') {
        isWeekendOrHoliday = true;
      } else if (statusSpesial === 'TUTUP') {
        isTutup = true;
      ...
      } else { // HARIAN_FULL - Harian Senin Tutup
        if (isTutup) {
          // No charge
        } else if (isWeekendOrHoliday) {
          totalBill += weekendRate;
  ...
  const handleGenerateExcel = () => {
    ...
    // Format menjadi string DD/MM/YYYY agar tidak mengandung jam
    const tglMulaiBayarStr = tglMulaiArr.length === 3 ? `${tglMulaiArr[2]}/${tglMulaiArr[1]}/${tglMulaiArr[0]}` : formGenerate.tglMulaiBayar;
    ...
    for(let R = 1; R <= range.e.r; ++R) { // Mulai dari baris 1 (lewati header)
      ...
      colsToText.forEach(C => {
        ...
        if(ws[cellAddress]) {
          ws[cellAddress].v = String(ws[cellAddress].v); // Konversi nilai ke string
      ...
      if(ws[dateCellAddress]) {
        ws[dateCellAddress].v = String(ws[dateCellAddress].v); // Konversi nilai ke string
        ws[dateCellAddress].t = 's'; // Pastikan tipenya string
        ws[dateCellAddress].z = '@'; // Pastikan formatnya text tanpa jam
      }
    }
              ...
              <main id="main-scroll-area" className="flex-1 overflow-y-auto p-4 lg:p-8 bg-slate-50/50 scroll-smooth relative">
                        ...
                        <table className="w-full text-left text-sm text-slate-600">
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {isDbLoading && merchants.length === 0 ? (
                              ...
                              let tagihanAktual = calculateMerchantBill(row);
                ...
                {userRole === 'admin' && activeMenu === 'kalender' && (
                  <div className="max-w-6xl mx-auto space-y-6">
                          ...
                          <div className="mt-6 pt-4 border-t border-slate-600 space-y-4">
                            ...
                            {tradeRates.length > 0 && (
                              <div className="pt-3 border-t border-slate-700 space-y-2.5">
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Simulasi Tarif Dagangan Khusus</p>
                                {tradeRates.map((rule) => {
                                  const simulatedRate = calculateMerchantBill({
                                    tipeTarif: rule.billingDays,
                                    jenisUsaha: rule.jenisUsaha
                                  });
                                  return (
                                    <div key={rule.id} className="bg-slate-950/40 p-2.5 rounded-lg border border-slate-750/30">
                                      <p className="text-[10px] text-amber-400 font-black uppercase tracking-wider">{rule.jenisUsaha}</p>
                                      <div className="flex justify-between items-center mt-1">
                                        <span className="text-[9px] text-slate-400 font-bold uppercase">
                                          {rule.billingDays === 'HARIAN_WEEKEND' ? 'Weekend Saja' : rule.billingDays === 'HARIAN_FULL_NONSTOP' ? 'Nonstop' : 'Senin Tutup'}
                                        </span>
                                        <span className="text-sm font-extrabold text-emerald-400">{formatRp(simulatedRate)}</span>
                                      </div>
                                    </div>
                                  );
                                })}
                    ...
                    {/* ATURAN TARIF KHUSUS JENIS DAGANGAN */}
                      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                        <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                          ...
                          <div>
                            <h4 className="text-base font-bold text-slate-800">Tarif Khusus Berdasarkan Jenis Dagangan</h4>
                            <p className="text-xs text-slate-500 mt-0.5">Tetapkan harga harian khusus atau pilih tipe penagihan khusus (Senin tutup, Nonstop, atau Weekend saja) untuk jenis dagangan tertentu.</p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                          ...
                          <div className="lg:col-span-1 bg-slate-50 p-5 rounded-xl border border-slate-200">
                            <h5 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-1.5">
                              <PlusCircle className="w-4 h-4 text-blue-600" />
                              Tambah / Ubah Aturan
                            </h5>
                            <form onSubmit={(e) => {
                              e.preventDefault();
                              if (!newTradeRate.jenisUsaha) {
                                showToast("Pilih atau masukkan jenis dagangan!", "error");
                                return;
                              }
                              const updatedRules = [...tradeRates];
                              const existingIdx = updatedRules.findIndex(r => r.jenisUsaha.trim().toUpperCase() === newTradeRate.jenisUsaha.trim().toUpperCase());
                              
                              const ruleData = {
                                id: existingIdx >= 0 ? updatedRules[existingIdx].id : 'rule_' + Date.now(),
                                jenisUsaha: newTradeRate.jenisUsaha.trim(),
                                billingDays: newTradeRate.billingDays,
                                tarifWeekday: Number(newTradeRate.tarifWeekday) || 0,
                                tarifWeekend: Number(newTradeRate.tarifWeekend) || 0
                              };

                              if (existingIdx >= 0) {
                                updatedRules[existingIdx] = ruleData;
                              } else {
                                updatedRules.push(ruleData);
                              }

                              handleSaveTradeRates(updatedRules);
                              setNewTradeRate({ jenisUsaha: '', billingDays: 'HARIAN_FULL', tarifWeekday: 10000, tarifWeekend: 15000 });
                            }} className="space-y-4">
                              <div>
                                <label className="block text-xs font-bold text-slate-600 mb-1">Jenis Dagangan *</label>
                                <input
                                  type="text"
                                  list="jenis-dagangan-suggestions"
                                  required
                                  value={newTradeRate.jenisUsaha}
                                  onChange={(e) => setNewTradeRate({ ...newTradeRate, jenisUsaha: e.target.value })}
                                  className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none text-sm bg-white focus:ring-2 focus:ring-blue-500"
                                  placeholder="Cari atau ketik baru (misal: Pecel)"
                                />
                                <datalist id="jenis-dagangan-suggestions">
                                  {uniqueJenisUsaha.map(ush => <option key={ush} value={ush} />)}
                                </datalist>
                              </div>

                              <div>
                                <label className="block text-xs font-bold text-slate-600 mb-1">Tipe Tagihan (Jadwal Aktif)</label>
                                <select
                                  value={newTradeRate.billingDays}
                                  onChange={(e) => {
                                    const val = e.target.value;
                                    let wday = newTradeRate.tarifWeekday;
                                    let wend = newTradeRate.tarifWeekend;
                                    if (val === 'HARIAN_WEEKEND') wday = 0;
                                    setNewTradeRate({ ...newTradeRate, billingDays: val, tarifWeekday: wday, tarifWeekend: wend });
                                  }}
                                  className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none text-sm bg-white cursor-pointer focus:ring-2 focus:ring-blue-500 font-medium text-slate-700"
                                >
                                  <option value="HARIAN_FULL">Harian (Senin Tutup)</option>
                                  <option value="HARIAN_FULL_NONSTOP">Harian (Nonstop / Senin dihitung)</option>
                                  <option value="HARIAN_WEEKEND">Weekend Saja (Sabtu, Minggu, Libur)</option>
                                </select>
                              </div>

                              <div className="grid grid-cols-2 gap-3">
                                <div>
                                  <label className="block text-xs font-bold text-slate-600 mb-1">Tarif Hari Biasa</label>
                                  <input
                                    type="number"
                                    min="0"
                                    value={newTradeRate.tarifWeekday}
                                    onChange={(e) => setNewTradeRate({ ...newTradeRate, tarifWeekday: parseInt(e.target.value) || 0 })}
                                    className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none text-sm bg-white focus:ring-2 focus:ring-blue-500 font-semibold"
                                  />
                                </div>
                                <div>
                                  <label className="block text-xs font-bold text-slate-600 mb-1">Tarif Weekend/Libur</label>
                                  <input
                                    type="number"
                                    min="0"
                                    value={newTradeRate.tarifWeekend}
                                    onChange={(e) => setNewTradeRate({ ...newTradeRate, tarifWeekend: parseInt(e.target.value) || 0 })}
                                    className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none text-sm bg-white focus:ring-2 focus:ring-blue-500 font-semibold"
                                  />
                                </div>
                              </div>

                              <button type="submit" className="w-full py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-bold shadow-md transition-colors flex justify-center items-center gap-2 mt-2">
                                <PlusCircle className="w-4 h-4" /> Simpan Aturan Tarif
                              </button>
                            </form>
                          </div>
                          ...
                          <div className="lg:col-span-2">
                            <h5 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-1.5">
                              ...
                              Daftar Aturan Aktif ({tradeRates.length})
                            </h5>
                            {tradeRates.length === 0 ? (
                              <div className="p-8 border border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center text-center text-slate-400">
                                <DollarSign className="w-10 h-10 text-slate-300 mb-2" />
                                <p className="text-sm font-medium">Belum ada tarif khusus jenis dagangan.</p>
                                <p className="text-xs text-slate-400 mt-0.5">Semua jenis dagangan ditagihkan tarif standar (Rp 10k weekday, Rp 15k weekend/libur).</p>
                              </div>
                            ) : (
                              <div className="border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                                <table className="w-full text-left text-sm text-slate-600">
                                  <thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-700 border-b border-slate-200">
                                    <tr>
                                      <th className="px-4 py-3">Jenis Dagangan</th>
                                      <th className="px-4 py-3">Tipe Tagihan</th>
                                      <th className="px-4 py-3">Tarif Biasa / Ramai</th>
                                      <th className="px-4 py-3 text-center">Aksi</th>
                                    </tr>
                                  </thead>
                                  <tbody className="divide-y divide-slate-100 bg-white">
                                    {tradeRates.map((rule, idx) => (
                                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                        <td className="px-4 py-3 align-middle font-bold text-slate-900 uppercase">
                                          {rule.jenisUsaha}
                                        </td>
                                        <td className="px-4 py-3 align-middle text-xs font-semibold">
                                          {rule.billingDays === 'HARIAN_WEEKEND' ? (
                                            <span className="px-2.5 py-1 bg-amber-50 text-amber-700 border border-amber-200 rounded-md">Weekend Saja</span>
                                          ) : rule.billingDays === 'HARIAN_FULL_NONSTOP' ? (
                                            <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-md">Harian Nonstop</span>
                                          ) : (
                                            <span className="px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-md">Harian (Senin Tutup)</span>
                                          )}
                                        </td>
                                        <td className="px-4 py-3 align-middle font-mono font-bold text-xs text-slate-800">
                                          {formatRp(rule.tarifWeekday)} / {formatRp(rule.tarifWeekend)}
                                        </td>
                                        <td className="px-4 py-3 align-middle text-center">
                                          <button
                                            type="button"
                                            onClick={() => {
                                              requestConfirm(`Cabut aturan tarif khusus untuk jenis dagangan "${rule.jenisUsaha}"?`, () => {
                                                const updated = tradeRates.filter(r => r.id !== rule.id);
                                                handleSaveTradeRates(updated);
                                              });
                                            }}
